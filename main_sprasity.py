...
if __name__ == "__main__":
    ...
    parser.add_argument('--sprasity', default=0.3, type=float, help='sprasity')
    args = parser.parse_args()
    seed_everything(args.seed)
    qos = np.load(f'data/rt_sprasity{args.sprasity}.npy') #t*u*s
    ...
    pkl.dump(rmse_of_k,open(f'./result/{args.dataset}_sprasity/rmse_of_k_{args.time_len}_{args.sprasity}_{args.R}.pkl','wb'))
    pkl.dump(mae_of_k,open(f'./result/{args.dataset}_sprasity/mae_of_k_{args.time_len}_{args.sprasity}_{args.R}.pkl','wb'))
    pkl.dump(conver_list, open(f'./result/{args.dataset}_sprasity/conver_list{args.time_len}_{args.sprasity}_{args.R}.pkl', 'wb'))
